...
func RateLimitExceeded(message string) *Response {
	if message == "" {
		message = GetMessage(RateLimitExceededCode)
	}
	return NewResponse(RateLimitExceededCode, message, nil)
}
